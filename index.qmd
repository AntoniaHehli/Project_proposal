...
```{r filter Posmo tracks to CH, message = FALSE, output = FALSE}
...
```{r plot all tracks, fig.cap = 'Posmo tracks for our analysis colored by transport mode.'}
...
```{r plot static, fig.cap = 'Static and non-static parts of the trajectories.'}
...
```{r known emission values depending on speed, fig.cap='Car CO~2~ emission values depending on speed (Umweltbundesamt, n.d.).'}
# plot them
ggplot()+
  geom_point(aes(x = speed, y = emission))+
  geom_line(aes(x = speed, y = emission))+
  labs(title = "Car CO~2~ emission values depending on speed",
       x = "Speed [km/h]",
       y = "CO2 Emission [g/km]")+
  theme_minimal()
```
...
```{r, fig.cap='Estimated car emission values based on speed. Values in red from Umweltbundesamt (n.d.).'}
...
ggplot()+
  ...
  labs(title = "Estimated car CO~2~ emission values depending on speed",
...
```{r plot height values for all posmo data, fig.cap='Elevation above sea level for posmo trajectories.'}
ggplot() + 
...
```{r total emissions simple, fig.cap='CO~2~ emission based on distances per user normalized over 7 days for different transport modes.'}
emissions_bar_simple <- ggplot(emissions, aes(x = user_id, y = co2_emissions_g_per_w, fill = transport_mode)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Users", y = "CO~2~ Emissions [g/week]", fill = "Transport Mode") +
...
```{r plot emissions simple, fig.cap='Spatial reference for CO~2~ emission based on distances per user normalized over 7 days for all transport modes.'}
emissions_plot_simple <- ggplot() + 
  ...
  labs(title = "Total Emissions per Transport Mode and User",
     x = "E",
     y = "N")+
  scale_color_continuous(name = "CO~2~ emissions [g/week]", type = "viridis")+
  scale_alpha_continuous(guide = "none") +
  coord_sf(datum = st_crs(2056))+
  theme_minimal()
...
```{r, fig.cap='CO~2~ emission based on distances per user normalized over 7 days for transport mode car.'}
...
plot_car_em_simple <- ggplot(emissions_car_simple, aes(x = user_id, y = co2_emissions_g_per_w, fill = user_id)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Users", y = "CO~2~ Emissions [g/week]", fill = "User",
       subtitle = "Car emissions calculated using distance") +
  ggtitle("Car Emissions") +
...
```{r, fig.cap = 'Car CO~2~ emission based on speed per user normalized over 7 days.'}
...
```{r, fig.cap = 'Comparison between CO~2~ emission of transport mode car using a simple distance-based scenario (left) and a scenario based on driving speed (right).}
...
```{r, fig.cap = 'Comparison between CO~2~ emission of all transports mode with mode car calculated using a simple distance-based scenario (left) and a scenario based on driving speed (right).'}
# create the bar chart
...
```{r plot locations with increase due to slope, fig.cap = 'Spatial reference of increase in car CO~2~ emission due to slope. For better visibility, increase values are limited to values between 0.5 and 10%.'}
posmo_car |> 
  ...
  labs(title = "Increase in emission due to slope",
       ...
       x = "E",
       y = "N")+
  scale_color_continuous(name = " CO~2~ emission increase [%]", type = "viridis")+
  coord_sf(datum = st_crs(2056))+
  theme_minimal()
```
...
```{r, fig.cap = 'Comparison car CO~2~ emission of three different scenarios. Top: distance-based, middle: speed-based, bottom: slope- and speed-based.'}
...
plot_car_em_slope <- ggplot(emissions_car_slope, aes(x = user_id, y = co2_emissions_g_per_w_slope, fill = user_id)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Users", y = "CO~2~ Emissions [g/week]", fill = "User",
       subtitle = "Car emissions calculated under consideration of slope") +
  ggtitle("Car Emissions") +
...
```{r, fig.cap = 'CO~2~ emission values for all transportat modes per user and week with car emissions calculated based on distance.'}
emissions_bar_simple + labs(subtitle = "Car emissions calculated based on distance")
```

```{r, fig.cap = 'CO~2~ emission values for all transportat modes per user and week with car emissions calculated based on speed.'}
emissions_plot_speed_car
```

```{r, fig.cap = 'CO~2~ emission values for all transportat modes per user and week with car emissions calculated based on slope and speed.'}
...
One thing that we did not include was the emission increase due to acceleration. Acceleration also has an influence on the emissions of cars and emissions change significantly with changing speed (Dong et al., 2022). In the figure below, we can see where there are locations with acceleration and where there are locations with deceleration.
