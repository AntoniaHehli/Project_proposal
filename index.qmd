...
posmo <- posmo %>%
  mutate(transport_mean = case_when(
    transport_mean == "E_Bike" ~ "E-Bike",
    transport_mean == "WheelChari" ~ "Wheel Chair",
    transport_mean == "Cable_Car" ~ "Cable Car"))
```
...
We filtered velocities which exceeded 200 km/h, which is the highest velocities some InterCity trains in Switzerland travel (Knupfer, 2014). We also set the minimum speed to 10 as this corresponds with the average velocity of our slowest transportation mode, bicycle.  
...
*restliche transport modes darstellen??* die sind dann im Vergleich, ansonsten haben wir das wieder doppelt. Oder habe ich etwas falsch verstanden?
...
When also including the emissions calculated using the slope and the resulting increase in carbon emissions the emission values for the speed and the slope approach are quite similar. For User 2 and 3 the emissions are higher with the slope approach than for the speed approach. *(maybe more hilly??)* Ich würde das in die Diskussion packen.
...
*vlt gibt posmo infos dazu wie emissionen berechet werden; vergleich mit eigenen annahmen.* bisher nichts gefunden
...
Interesting to see is that the car emissions are lower when increasing the complexity of our model. When we include speed and slope in our calculations the CO~2~ emissions decrease although we expected them to increase. This may be due to the fact that we have more data points with slower car speed and only little inclinations. Slower speeds were connected with a lower emission value compared to when using one constant emission value over the whole distance. The same goes for the slope and speed where we have a lot of data points with only minor inclinations and therefore the emission values for these are lower than for the constant car emission vaue of the simple scenario. 
