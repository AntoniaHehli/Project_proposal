...
*ich habe in dem Chunk da unten "-Airplane" rausgenommen, das gab einen Fehler irgendwie*
...
emission_posmo_mode <- posmo_emission |>
  tidyr::gather(transport_mode, emission, -user_id, -Date, -Notes, -Sum_all, -Sum_CH) |>
  mutate(origin = "posmo") |>
...
*Ich habe in diesem Plot gar nichts gesehen, nur weiss und Achsen*
...
For User 2 and 3 the emissions are higher with the slope approach than for the speed approach. This may be due to the fact that they were driving on more hilly roads with probably a higher speed compared to user 1.
...
*evtl wäre vorher mal noch eine abbildung gut wo wir sagen was die zurückgelegten distanzen sind pro verkehrsmittel um etw. relativieren zu können* Gute Idee! Kann ich auch versuchen mir am Abend anzuschauen (Antonia). *Das ist erledigt, du darfst gerne schieben, wohin du willst.*

```{r distance per user and transport mode, output = TRUE}
#| label: fig-twentiethFigure
#| fig-cap: Overview over travelled distance per user and transport mode.
ggplot(posmo, aes(x = user_id, y = distance_km, fill = transport_mode))+
  geom_bar(stat = "identity", position = "stack") +
  labs(x = "User", 
       y = expression("Distance [km]"),
       fill = "Transport mode") +
  ggtitle("Distance travelled per User and Transport Mode") +
  theme_minimal()+
  theme(axis.text.x = element_blank())
```
