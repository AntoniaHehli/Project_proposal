...
```{r define function loadPosmoData and load data}
...
*braucht es diese Abbildung? plus/minus die selbe wie oben*
...
In order to determine the influence of the complexity of an approach we also considered other factors such as the effect of velocity or slope on the carbon emissions. These additional factors were only calculated and compared for the emissions of cars.
Speed
Information about the change in emission for velocities between 100 and 130 km/h were provided by the Umweltbundesamt (2023). To estimate the emission for lower velocities we used a non-linear least squares regression. We assumed that the reduction in carbon emissions decreases at lower velocities and therefore set a threshold at 75 km/h where the carbon emissions are constant (Metcalfe & Boulter, 2022).
...
*brauchen wir get_days function oder eigentlich nur get_weeks?? könnte man Funktion umschreiben?*
...
*braucht es das oder reicht untere darstellung?*
...
```{r plot known and interpolated emission values depending on speed}
...
*Darstellung eher mit kontinuierlicher Skala anstatt mit einzelnen punkten (falls möglich)*
...
```{r filter slope and extrapolate emission values depending on slope}
...
```{r calculate emission values considering slope}
...
```{r plot weekly emissions for simple approach}
...
```{r plot weekly emissions for simple approach on map}
...
*nicht bereits vorhin nach car gefilteret?? braucht es das nochmals?*

```{r plot overall emission of car for simple approach}
...
```{r plot overall emission of car for speed approach}
...
```{r plot overall emission of car for comparison of both approaches}
...
```{r plot average weekly emission}
# create the bar chart
...
```{r plot overall emission of car for comparison of all three approaches}
...
```{r plot comparison between our calculated emission and the Posmo emission}
...
Our analysis is prone to several limitations. These range from inaccuracies in the data to limitations in our methods. 
Although we worked with only validated data from the Posmo app, there were inaccuracies present. For example, sometimes speed values were exceptionally high or there was a time lag of 0 between two data points but they were at different locations. These inaccuracies led us to exclude all the outliers which led to a reduced amount of data and assumptions we had to make.
