...
We only need transportation modes that generate carbon dioxide emissions. In our analysis we include car, bus, train, and tram, bikes and e bikes. As we are only going to focus on trajectories in Switzerland, we do not include airplanes in our considerations.
...
# KÖNNTE MAN EVTL AUCH RAUSNEHMEN, AUSSER MAN WILL DIES NOCH FÜR GRAPHIK NUTZEN
...
weekly_emission <- weekly_emissions(posmo_CH, constants_emission)
```

#### Slope

```{r plot height values for all posmo data}
ggplot(posmo_CH, aes(color = height)) + 
  geom_sf() +
  coord_sf(datum = st_crs(2056))+
  labs(x = "E",
       y = "N",
...
posmo_CH_car_fast <- posmo_CH_car |> 
  group_by(user_id)|>
...
posmo_CH_car_slow <- posmo_CH_car |>
  group_by(user_id)|>
  filter(speed_kmh_floored <= 75) |>
...
posmo_CH_car <- rbind(posmo_CH_car_fast, posmo_CH_car_slow)
```
...
# sum up per user
posmo_CH_car_emissions <- posmo_CH_car_traveldistance |>
  group_by(user_id)|>
  summarise(sum_emissions_g = sum(emissions_g, na.rm = TRUE))

# join with the number of days & weeks per user
posmo_CH_car_emissions <- posmo_CH_car_emissions |> left_join(posmo_CH_days_per_user)

# calculate emissions per week
posmo_CH_car_emissions <- posmo_CH_car_emissions |>
  group_by(user_id)|>
...
```{r}
# calculate the total emissions by user ID and transportation mode
emissions_car_CH <- aggregate(sum_emissions_g_w ~ user_id, posmo_CH_car_emissions, sum)

# create the bar chart
ggplot(emissions_car_CH, aes(x = user_id, y = sum_emissions_g_w, fill = user_id)) +
  geom_bar(stat = "identity", position = "dodge") +
...
daily_emission_CH <- daily_emissions(posmo_CH, constants_emission)
```
...
emission_avg <- aggregate(avg_emission_kg ~ user_id, daily_emission_CH, sum)
emission_min <- aggregate(min_emission_kg ~ user_id, daily_emission_CH, sum)
emission_max <- aggregate(max_emission_kg ~ user_id, daily_emission_CH, sum)
