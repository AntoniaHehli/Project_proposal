...
```{r}
## versuch nur emission für slope ohne speed zu berechnen; geht das?

test_emissions_car_slope <- emissions_car_slope
test_emissions_car_slope$test_gkm <- 116.41017

#test_emissions_car_slope <- test_emissions_car_slope |>
#  mutate(test_emission_slope = test_gkm * (1 + mean_increase_perc/100))

summary(test_emissions_car_slope)
```
...
*Ich versuche da noch die Labels für die User zu fixen.*

```{r plot car tracks above DHM, output = TRUE}
# convert the car data to LV03 as the DHM is LV03
posmo_car_lv03 <- posmo_lv03 |>
  filter(transport_mode == "Car")

# get the coordinates of this newly created data set
posmo_car_lv03_coord <- st_coordinates(posmo_car_lv03)

# add them back to the data frame
posmo_car_lv03$X <- posmo_car_lv03_coord[,1]
posmo_car_lv03$Y <- posmo_car_lv03_coord[,2]

#| label: fig-thirdteenthFigure
#| fig-cap: Posmo tracks overlaid on the DHM25.
# plot the dhm with the trajectories on top of it
ggplot()+
  geom_spatraster(data = dhm25)+
  theme_minimal()+
  labs(x = "E",
       y = "N",
       title = "Posmo car tracks over the DHM",
       col = "User",
       fill = "Elevation (m.a.s.l)")+
  geom_sf(data = posmo_car_lv03, aes(col = user_id))+
