...
# make the last join and convert to a sf object and convert to Swiss coordinates
posmo_joined <- full_join(posmo_joined, posmo6)

# remove rows with NA in the coordinates
posmo_joined <- posmo_joined[!is.na(posmo_joined$lon_x), ]

# convert to sf object and to Swiss coordinates
posmo_joined <- posmo_joined |>
  st_as_sf(coords = c("lon_x", "lat_y"), crs = 4326) |> 
  st_transform(2056)

# extract coordinates
posmo_coord <- st_coordinates(posmo_joined)

# create one dataframe
posmo_joined <- cbind(posmo_joined, posmo_coord)
```
